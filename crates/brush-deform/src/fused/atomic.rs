//! f32-atomic-add abstraction: native `Atomic<f32>::fetch_add` when the
//! device supports it, an `Atomic<u32>` CAS fallback otherwise.
//! Mirrors `brush-render-bwd` / `brush-voxel` (small private copy).
