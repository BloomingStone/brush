//! End-to-end autodiff test: `brush_voxel::voxelize` produces a density
//! volume that backprops through burn's autodiff graph to `transforms`
//! and `raw_opacities`, and those grads match the raw
//! `voxelize_bwd_pipeline` (already validated against R2-Gaussian).

use brush_cube::MainBackendBase;
use brush_xray::XRaySplats;
use burn::backend::ops::FloatTensorOps;
use burn::tensor::TensorData;
use brush_voxel::{VoxelSettings, voxelize, voxelize_bwd_pipeline};
...
async fn autodiff_grads_match_pipeline() {
    ...
    // Deterministic scene (same RNG as the R2 golden reference).
    let n = 32usize;
    ...
    let vol = voxelize(splats.clone(), &settings).await;
    assert_eq!(vol.dims(), [16, 16, 16]);
    let grads = vol.sum().backward();
    ...
    // Reference: raw pipeline with dL/dvol = 1 (L = vol.sum()).
    ...
    let v_volume_ft =
        MainBackendBase::float_from_data(TensorData::ones::<f32, _>([16, 16, 16]), &base);

    let ref_grads = voxelize_bwd_pipeline(&settings, transforms_ft, raw_opac_ft, v_volume_ft)
        .await;
    ...
    // Both are interleaved [N,10] = means(3)+quats(4)+log_scales(3).
